...
/* This code uses an algorithm protected by U.S. Patent #4,405,829
   ...
   patent into the publickey domain on Sep 6th, 2000.
*/
...
typedef struct {
    MPI n;	    /* publickey modulus */
    MPI e;	    /* publickey exponent */
    MPI d;	    /* exponent */
    MPI p;	    /* prime  p. */
    MPI q;	    /* prime  q. */
...
static void publickey(MPI output, MPI input, RSA_publickey_key *skey );
static void secret(MPI output, MPI input, RSA_secret_key *skey );

static void test_keys( RSA_secret_key *sk, unsigned nbits )
{
    RSA_publickey_key pk;
    ...
    publickey( out1, test, &pk );
    secret( out2, out1, sk );
      
    secret( out1, test, sk );
    publickey( out2, out1, &pk );
...
generate( RSA_secret_key *sk, unsigned nbits )
{
    ...
    phi = mpi_alloc ( mpi_nlimb_hint_from_nbits (nbits) );
    g	= mpi_alloc ( mpi_nlimb_hint_from_nbits (nbits) );
    f	= mpi_alloc ( mpi_nlimb_hint_from_nbits (nbits) );
    mpi_sub_ui( t1, p, 1 );
    mpi_sub_ui( t2, q, 1 );
    mpi_mul( phi, t1, t2 );
    mpi_gcd(g, t1, t2);
 ...
 */
static void
publickey(MPI output, MPI input, RSA_publickey_key *pkey )
{
...
#else
    MPI m1   = mpi_alloc( mpi_get_nlimbs(skey->n)+1 );
    MPI m2   = mpi_alloc( mpi_get_nlimbs(skey->n)+1 );
    MPI h    = mpi_alloc( mpi_get_nlimbs(skey->n)+1 );
...
rsa_generate( int algo, unsigned nbits, MPI *skey, MPI **retfactors )
{
 ...
 //   MYDEBUG(1);
    ...
    if (retfactors)
      *retfactors = (MPI *)malloc( 1 * sizeof **retfactors );
    return 0;
}
...
rsa_verify( int algo, MPI hash, MPI *data, MPI *pkey )
{
    ...
    publickey( result, data[0], &pk );
    rc = mpi_cmp( result, hash ) ? -1:0;
    mpi_free(result);
